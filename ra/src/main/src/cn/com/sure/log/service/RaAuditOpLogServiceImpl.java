...
import cn.com.sure.log.dao.RaAuditOpLogDAO;
import cn.com.sure.log.entry.RaAuditOpLog;

@Transactional(propagation = Propagation.REQUIRED)
@Service(value="auditOpLogService")
public class RaAuditOpLogServiceImpl implements RaAuditOpLogService{
	
	private static final Log LOG = LogFactory.getLog(RaAuditOpLogServiceImpl.class);
	
	@Autowired
	private RaAuditOpLogDAO auditOpLogDAO;
	 ...
	 */
	@Override
	public List<RaAuditOpLog> selectAll() {
		LOG.debug("selectAll - start");
		List<RaAuditOpLog> auditOpLogs = auditOpLogDAO.selectAll();
	 ...
	 */
	@Override
	public List<RaAuditOpLog> searchByCondition(RaAuditOpLog auditOpLog) {
		LOG.debug("searchByCondition - start");
		List<RaAuditOpLog> auditOpLogs = auditOpLogDAO.searchByCondition(auditOpLog);
		LOG.debug("searchByCondition - start");
		return auditOpLogs;
	}
	...
	public void insert(long type, String action, String actionExt1,
		...
		RaAuditOpLog auditOpLog = new RaAuditOpLog();
	 ...
	 */
	@Override
	public void exportExcel(String[] titles, ServletOutputStream out,RaAuditOpLog auditOpLog) throws Exception {
		...
		hssfCell.setCellStyle(hssfCellStyle);//列居中显示                
		 ...
		 if(auditOpLogs != null && !auditOpLogs.isEmpty()){
		     for (int i = 0; i < auditOpLogs.size(); i++) {
		         ...
		         RaAuditOpLog opLog = auditOpLogs.get(i);
		      // 第六步，创建单元格，并设置值
