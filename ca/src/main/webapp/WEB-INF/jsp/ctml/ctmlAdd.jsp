	               	  ...
	               	  <div class="row"> 
		                  ...
		                  <div class="col-md-6 margin-bottom-15">
		                    ...
		                    <select class="form-control margin-bottom-15" name="validity.id" id="validity" value="${certTemplate.validity}" required="required">
		                    	...
		                    	<c:forEach var="cd" items="${code}">
		                    		<option value="${cd.id}">${cd.paraValue}</option>
		                    	</c:forEach>
		                    </select>
		                  </div>
				      </div>
		                    ...
		                    <label for="kpgAlgorithm" class="control-label">密钥类型</label>
		                      <select class="form-control margin-bottom-15" name="kpgAlgorithm.id" id="kpgAlgorithm" value="${certTemplate.kpgAlgorithm}" required="required">
		                    	...
		                    	<c:forEach var="sc" items="${sysCodes}">
		                    		<option value="${sc.id}">${sc.paraValue}</option>
		                    	</c:forEach>
		                    </select>
		                  </div>
		                  <div class="col-md-6 margin-bottom-15">
		                    <label for="isSingle" class="control-label">证书类型</label>
		                    	<select class="form-control margin-bottom-15" name="isSingle" id="isSingle" required="required" >
							      <option value="2">双证书</option>
							      <option value="1">单证书</option>
		                  	   </select>                
		                  </div>
				     </div> 
	                  ...
	                  <div class="col-md-6 margin-bottom-15">
	                    <label for="notes" class="control-label">加密证书用法扩展</label>
		                   <div class="row">
			                <div class="col-md-12 margin-bottom-15">
			                  <label class="checkbox-inline" required="required">
					                 <c:forEach var="sceeu" items="${sysCodeEncExtUse}">
