...
import cn.com.sure.log.dao.CaAuditOpLogDAO;
import cn.com.sure.log.entry.CaAuditOpLog;

@Transactional(propagation = Propagation.REQUIRED)
@Service(value="auditOpLogService")
public class CaAuditOpLogServiceImpl implements CaAuditOpLogService{
	
	private static final Log LOG = LogFactory.getLog(CaAuditOpLogServiceImpl.class);
	
	@Autowired
	private CaAuditOpLogDAO auditOpLogDAO;
	 ...
	 */
	@Override
	public List<CaAuditOpLog> selectAll() {
		LOG.debug("selectAll - start");
		List<CaAuditOpLog> auditOpLogs = auditOpLogDAO.selectAll();
	 ...
	 */
	@Override
	public List<CaAuditOpLog> searchByCondition(CaAuditOpLog auditOpLog) {
		LOG.debug("searchByCondition - start");
		List<CaAuditOpLog> auditOpLogs = auditOpLogDAO.searchByCondition(auditOpLog);
		LOG.debug("searchByCondition - start");
		return auditOpLogs;
	}
	...
	public void insert(long type, String action, String actionExt1,
		...
		CaAuditOpLog auditOpLog = new CaAuditOpLog();
	 ...
	 */
	@Override
	public void exportExcel(String[] titles, ServletOutputStream out,CaAuditOpLog auditOpLog) throws Exception {
		...
		hssfCell.setCellStyle(hssfCellStyle);//列居中显示                
		 ...
		 if(auditOpLogs != null && !auditOpLogs.isEmpty()){
		     for (int i = 0; i < auditOpLogs.size(); i++) {
		         ...
		         CaAuditOpLog opLog = auditOpLogs.get(i);
		      // 第六步，创建单元格，并设置值
